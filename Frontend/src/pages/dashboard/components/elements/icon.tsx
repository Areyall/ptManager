import React from 'react';

interface Props {
  icon: string;
  size: string;
}

function Icon({ icon, size }: Props) {
  if (icon === 'bug') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          fill="#000000"
          height={size}
          width={size}
          version="1.1"
          id="Capa_1"
          viewBox="0 0 511 511"
          xmlSpace="preserve"
        >
          <g>
            <path
              d="M327.5,0C226.318,0,144,82.318,144,183.5c0,47.899,18.455,91.563,48.619,124.275l-23.391,23.391
		c-0.468,0.468-0.853,0.986-1.172,1.532C163.131,329.642,157.443,328,151.5,328c-8.417,0-16.327,3.278-22.272,9.228L9.229,457.227
		C3.278,463.176,0,471.086,0,479.5s3.278,16.324,9.226,22.27C15.173,507.722,23.083,511,31.5,511s16.327-3.278,22.272-9.228
		l119.999-119.999c5.951-5.949,9.229-13.859,9.229-22.273c0-5.943-1.642-11.631-4.698-16.556c0.546-0.319,1.065-0.703,1.533-1.172
		l23.391-23.391C235.937,348.545,279.601,367,327.5,367C428.682,367,511,284.682,511,183.5S428.682,0,327.5,0z M43.163,491.167
		C40.05,494.284,35.908,496,31.5,496s-8.55-1.716-11.667-4.835C16.716,488.049,15,483.906,15,479.5s1.716-8.549,4.834-11.666
		l106.697-106.697l23.331,23.331L43.163,491.167z M163.166,371.166l-2.697,2.697l-23.331-23.331l2.699-2.699
		c3.113-3.116,7.255-4.833,11.663-4.833s8.55,1.716,11.667,4.835c3.117,3.116,4.833,7.258,4.833,11.665
		S166.284,368.049,163.166,371.166z M327.5,352C234.589,352,159,276.411,159,183.5S234.589,15,327.5,15S496,90.589,496,183.5
		S420.411,352,327.5,352z"
            />
            <path
              d="M473.064,141.375c-9.877-34.188-31.68-63.969-61.392-83.858c-3.442-2.305-8.1-1.382-10.404,2.061
		c-2.304,3.442-1.382,8.1,2.061,10.404c26.779,17.926,46.427,44.759,55.325,75.556c0.949,3.284,3.946,5.42,7.202,5.42
		c0.689,0,1.39-0.096,2.085-0.297C471.92,149.512,474.214,145.354,473.064,141.375z"
            />
            <path
              d="M312.324,319.167c-29.982-3.314-57.389-16.145-79.259-37.105c-2.99-2.866-7.738-2.765-10.604,0.225
		c-2.866,2.99-2.766,7.738,0.225,10.604c23.926,22.931,55.175,37.558,87.989,41.185c0.28,0.031,0.558,0.046,0.833,0.046
		c3.772,0,7.021-2.839,7.445-6.676C319.409,323.328,316.44,319.622,312.324,319.167z"
            />
            <path
              d="M206.171,246.107c-1.902-3.679-6.428-5.12-10.107-3.217c-3.679,1.902-5.12,6.428-3.217,10.107
		c2.601,5.029,5.519,9.969,8.675,14.683c1.447,2.161,3.821,3.329,6.239,3.329c1.434,0,2.884-0.411,4.166-1.269
		c3.442-2.304,4.364-6.963,2.06-10.405C211.143,255.088,208.513,250.638,206.171,246.107z"
            />
            <path
              d="M357.843,50.383c4.93,1.119,9.851,2.53,14.625,4.195c0.817,0.285,1.65,0.42,2.47,0.42c3.102,0,6.003-1.938,7.082-5.032
		c1.364-3.911-0.701-8.188-4.612-9.551c-5.304-1.85-10.77-3.417-16.246-4.66c-4.038-0.918-8.057,1.614-8.974,5.654
		C351.272,45.448,353.804,49.466,357.843,50.383z"
            />
            <path
              d="M414.854,174.029c-0.422-2.11-1.729-3.937-3.59-5.016c-1.861-1.08-4.095-1.308-6.136-0.628l-15.764,5.255
		c-1.168-6.344-2.942-12.397-5.245-18.064l10.735-5.368c1.452-0.726,2.628-1.902,3.354-3.354l16-32
		c1.853-3.705,0.351-8.21-3.354-10.062c-3.705-1.853-8.21-0.351-10.062,3.354L385.91,137.91l-8.633,4.317
		c-6.531-10.319-15.05-18.576-24.84-23.831l13.771-27.542c1.853-3.705,0.351-8.21-3.354-10.062
		c-3.703-1.853-8.209-0.351-10.062,3.354l-14.512,29.023c-3.507-0.754-7.104-1.169-10.78-1.169s-7.274,0.415-10.78,1.169
		l-14.512-29.023c-1.852-3.705-6.356-5.208-10.062-3.354c-3.705,1.852-5.207,6.357-3.354,10.062l13.771,27.542
		c-9.79,5.254-18.309,13.512-24.84,23.831l-8.633-4.317l-14.882-29.764c-1.852-3.705-6.357-5.207-10.062-3.354
		c-3.705,1.852-5.207,6.357-3.354,10.062l16,32c0.726,1.452,1.902,2.628,3.354,3.354l10.735,5.368
		c-2.303,5.667-4.077,11.72-5.245,18.064l-15.764-5.255c-2.042-0.68-4.275-0.451-6.136,0.628c-1.86,1.08-3.168,2.906-3.59,5.016
		l-8,40c-0.812,4.062,1.822,8.013,5.884,8.825c0.497,0.099,0.991,0.147,1.479,0.147c3.502,0,6.633-2.465,7.347-6.031l6.333-31.669
		l10.864,3.621c-0.022,0.858-0.052,1.713-0.052,2.577c0,8.995,1.217,17.636,3.426,25.705c-0.435,0.282-0.848,0.611-1.229,0.992
		l-16,16c-2.434,2.434-2.898,6.211-1.128,9.162l24,40c1.406,2.344,3.89,3.643,6.438,3.642c1.313,0,2.644-0.345,3.852-1.07
		c3.552-2.131,4.704-6.738,2.572-10.29l-20.987-34.979l8.17-8.17C284.244,255.55,304.456,271,327.5,271s43.256-15.45,54.386-38.508
		l8.17,8.17l-20.987,34.979c-2.131,3.552-0.979,8.159,2.572,10.29c1.208,0.725,2.539,1.07,3.852,1.07
		c2.548,0,5.032-1.298,6.438-3.642l24-40c1.771-2.951,1.306-6.729-1.128-9.162l-16-16c-0.381-0.381-0.794-0.71-1.229-0.992
		c2.209-8.069,3.426-16.71,3.426-25.705c0-0.864-0.03-1.719-0.052-2.577l10.864-3.621l6.333,31.669
		c0.713,3.565,3.845,6.031,7.347,6.031c0.487,0,0.982-0.048,1.479-0.147c4.062-0.813,6.696-4.764,5.884-8.825L414.854,174.029z
		 M320,255.226c-23.194-4.809-41-31.552-41-63.726c0-10.042,1.736-19.555,4.828-28.038c10.484,6.309,22.908,10.177,36.172,11.236
		V255.226z M290.47,149.911C299.374,135.912,312.666,127,327.5,127s28.126,8.912,37.03,22.911C354.26,156.366,341.113,160,327.5,160
		S300.74,156.366,290.47,149.911z M335,255.226v-80.527c13.264-1.06,25.688-4.928,36.172-11.236
		c3.092,8.483,4.828,17.995,4.828,28.038C376,223.674,358.194,250.417,335,255.226z"
            />
          </g>
        </svg>
      </div>
    );
  }
  if (icon === 'req') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width={size}
          height={size}
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          stroke="#000000"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="miter"
        >
          <polygon
            points="22 2 22 16 14 16 8 21 8 16 2 16 2 2 22 2"
            fill="#059cf7"
            opacity="0.1"
            strokeWidth="0"
          ></polygon>
          <polygon points="22 2 22 16 14 16 8 21 8 16 2 16 2 2 22 2"></polygon>
          <line x1="12" y1="6" x2="12" y2="12"></line>
          <line x1="15" y1="9" x2="9" y2="9"></line>
        </svg>
      </div>
    );
  }
  if (icon === 'stat') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width={size}
          height={size}
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M16 16L16 8"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M12 16L12 11"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M8 16L8 13"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M3 20.4V3.6C3 3.26863 3.26863 3 3.6 3H20.4C20.7314 3 21 3.26863 21 3.6V20.4C21 20.7314 20.7314 21 20.4 21H3.6C3.26863 21 3 20.7314 3 20.4Z"
            stroke="#000000"
            strokeWidth="1.5"
          />
        </svg>
      </div>
    );
  }
  if (icon === 'graph') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width={size}
          height={size}
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M20 20H4V4"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M4 16.5L12 9L15 12L19.5 7.5"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </svg>
      </div>
    );
  }
  if (icon === 'journal') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width={size}
          height={size}
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M6 6L18 6"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M6 10H18"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M12 14L18 14"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M12 18L18 18"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M2 21.4V2.6C2 2.26863 2.26863 2 2.6 2H21.4C21.7314 2 22 2.26863 22 2.6V21.4C22 21.7314 21.7314 22 21.4 22H2.6C2.26863 22 2 21.7314 2 21.4Z"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M6 18V14H8V18H6Z"
            fill="#000000"
            stroke="#000000"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </svg>
      </div>
    );
  }
  if (icon === 'check') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width={size}
          height={size}
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          stroke="#000000"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="miter"
        >
          <rect
            x="2"
            y="4"
            width="20"
            height="18"
            fill="#059cf7"
            opacity="0.1"
            rx="0"
          ></rect>
          <rect x="2" y="4" width="20" height="18" rx="0"></rect>
          <line x1="7" y1="2" x2="7" y2="6"></line>
          <line x1="17" y1="2" x2="17" y2="6"></line>
          <polyline points="7.5 13.5 10 16.5 16 11"></polyline>
        </svg>
      </div>
    );
  }
  if (icon === 'decline') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width={size}
          height={size}
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          stroke="#000000"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="miter"
        >
          <polygon
            points="22 2 22 16 14 16 8 21 8 16 2 16 2 2 22 2"
            fill="#059cf7"
            opacity="0.1"
            strokeWidth="0"
          ></polygon>
          <polygon points="22 2 22 16 14 16 8 21 8 16 2 16 2 2 22 2"></polygon>
          <line x1="15" y1="9" x2="9" y2="9"></line>
        </svg>
      </div>
    );
  }
  if (icon === 'aprove') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width={size}
          height={size}
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          stroke="#000000"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="miter"
        >
          <path d="M8,12.44V21l4-2,4,2V12.44" strokeLinecap="round"></path>
          <circle cx="12" cy="8" r="6"></circle>
        </svg>
      </div>
    );
  }
  if (icon === 'date') {
    return (
      <div className="pr-1 pt-[2px]">
        <svg
          width="18px"
          height="18px"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M20 10V7C20 5.89543 19.1046 5 18 5H6C4.89543 5 4 5.89543 4 7V10M20 10V19C20 20.1046 19.1046 21 18 21H6C4.89543 21 4 20.1046 4 19V10M20 10H4M8 3V7M16 3V7"
            stroke="gray"
            strokeWidth="2"
            strokeLinecap="round"
          />
        </svg>
      </div>
    );
  }
  if (icon === 'jobtype') {
    return (
      <div className="pr-2">
        <svg
          width="20px"
          height="20px"
          viewBox="0 0 60 60"
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M28,27 L28,30 L32,30 L32,27 L32,22 L28,22 L28,27 Z M8,27 C8,26.448 8.448,26 9,26 L26,26 L26,22 C26,20.897 26.897,20 28,20 L32,20 C33.103,20 34,20.897 34,22 L34,26 L51,26 C51.552,26 52,26.448 52,27 C52,27.552 51.552,28 51,28 L34,28 L34,30 C34,31.103 33.103,32 32,32 L28,32 C26.897,32 26,31.103 26,30 L26,28 L9,28 C8.448,28 8,27.552 8,27 L8,27 Z M30.03,29 C30.583,29 31.03,28.552 31.03,28 C31.03,27.448 30.583,27 30.03,27 L30.02,27 C29.468,27 29.025,27.448 29.025,28 C29.025,28.552 29.478,29 30.03,29 L30.03,29 Z M7,10 C6.449,10 6,10.449 6,11 L6,40 L54,40 L54,11 C54,10.449 53.551,10 53,10 L7,10 Z M26,8 L34,8 L34,6 L26,6 L26,8 Z M22,8 L24,8 L24,5 C24,4.448 24.448,4 25,4 L35,4 C35.552,4 36,4.448 36,5 L36,8 L38,8 L38,3 C38,2.449 37.551,2 37,2 L23,2 C22.449,2 22,2.449 22,3 L22,8 Z M7,8 L20,8 L20,3 C20,1.346 21.346,0 23,0 L37,0 C38.654,0 40,1.346 40,3 L40,8 L53,8 C54.654,8 56,9.346 56,11 L56,41 C56,41.552 55.552,42 55,42 L5,42 C4.448,42 4,41.552 4,41 L4,11 C4,9.346 5.346,8 7,8 L7,8 Z M39,54 L36,54 L36,53 C36,52.448 35.552,52 35,52 C34.448,52 34,52.448 34,53 L34,55 C34,55.552 34.448,56 35,56 L39,56 C40.123,56 41.295,56.914 41.775,58 L18.225,58 C18.705,56.914 19.877,56 21,56 L31,56 C31.552,56 32,55.552 32,55 C32,54.448 31.552,54 31,54 L26,54 L26,53 C26,52.448 25.552,52 25,52 C24.448,52 24,52.448 24,53 L24,54 L21,54 C18.43,54 16,56.43 16,59 C16,59.552 16.448,60 17,60 L43,60 C43.552,60 44,59.552 44,59 C44,56.43 41.57,54 39,54 L39,54 Z M30.02,44 C29.468,44 29.025,44.448 29.025,45 C29.025,45.552 29.478,46 30.03,46 C30.583,46 31.03,45.552 31.03,45 C31.03,44.448 30.583,44 30.03,44 L30.02,44 Z M60,9 L60,45 C60,47.804 57.804,50 55,50 L5,50 C1.898,50 0,45.468 0,43 L0,9 C0.191,6.167 2.386,4 5,4 L17,4 C17.552,4 18,4.448 18,5 C18,5.552 17.552,6 17,6 L5,6 C3.252,6 2.101,7.56 1.998,9.068 L2,43 C2,45.02 3.58,48 5,48 L55,48 C56.71,48 58,46.71 58,45 L58,9 C58,7.346 56.654,6 55,6 L43,6 C42.448,6 42,5.552 42,5 C42,4.448 42.448,4 43,4 L55,4 C57.757,4 60,6.243 60,9 L60,9 Z"
            id="job-desktop"
          ></path>
        </svg>
      </div>
    );
  }
  if (icon === 'glob') {
    return (
      <div className="ml-[2px] pr-1">
        <svg
          fill="#000000"
          width={size}
          height={size}
          viewBox="0 0 16 16"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M8 .5A7.76 7.76 0 0 0 0 8a7.76 7.76 0 0 0 8 7.5A7.76 7.76 0 0 0 16 8 7.76 7.76 0 0 0 8 .5zm6.71 6.8L13.48 7c-.25-.07-.27-.09-.29-.12-.15-.2-.32-.47-.48-.73 0-.09-.13-.23-.16-.31s.35-.6.51-.84a2.43 2.43 0 0 1 .59-.45 5.87 5.87 0 0 1 1.06 2.75zM8 1.75l-.09.17a.19.19 0 0 1 0-.1c0 .06-.15.15-.25.25l-.3.29a.85.85 0 0 0-.08 1.08h-.12a1.05 1.05 0 0 0-.81.42 1.27 1.27 0 0 0-.2 1.07V5a3 3 0 0 0-.43.11l-.24.08-.64.21a1.2 1.2 0 0 0-.81.8 1 1 0 0 0 .2.93 5.67 5.67 0 0 0 1.38 1.09 4.17 4.17 0 0 0 1.67.65h1.68a1.2 1.2 0 0 1 1.04.51.49.49 0 0 1 .13.43.77.77 0 0 1-.15.35 2.71 2.71 0 0 0-.95 1.61 11.11 11.11 0 0 1-.48 1.38c-.12.31-.23.61-.31.85a3.32 3.32 0 0 1-1-.08 3.28 3.28 0 0 0-.5-2.12 2.24 2.24 0 0 1-.53-1.42 2.11 2.11 0 0 0-1.47-2.29 10.81 10.81 0 0 1-2.9-2.64A6.79 6.79 0 0 1 8 1.75zM1.25 8a5.64 5.64 0 0 1 .12-1.16 10.29 10.29 0 0 0 2.94 2.42c.6.22.69.45.69 1.12a3.45 3.45 0 0 0 .86 2.27A3.05 3.05 0 0 1 6 14a6.35 6.35 0 0 1-4.75-6zm8.32 6.08c0-.15.12-.32.18-.48a10.2 10.2 0 0 0 .55-1.6 1.55 1.55 0 0 1 .54-.86 1.91 1.91 0 0 0 .57-1.3 1.71 1.71 0 0 0-.47-1.27 2.45 2.45 0 0 0-2-.9H7.35a4.77 4.77 0 0 1-2-1.11l.47-.16.27-.08a.79.79 0 0 1 .38-.07l.09.15a.64.64 0 0 0 .81.29.65.65 0 0 0 .34-.8v-.18c-.11-.3-.24-.72-.32-1A1.42 1.42 0 0 0 8.68 4a1 1 0 0 0-.18-1 3.44 3.44 0 0 0 .33-.34 1 1 0 0 0 .22-.8 6.93 6.93 0 0 1 3.73 1.8 3 3 0 0 0-.79.7 9.14 9.14 0 0 0-.64 1.09 1.46 1.46 0 0 0 .24 1.39c.18.31.38.61.56.86a1.58 1.58 0 0 0 1 .58c.22.06 1 .22 1.55.33a6.44 6.44 0 0 1-5.13 5.47z" />
        </svg>
      </div>
    );
  }
  if (icon === 'plus') {
    return (
      <div className="pr-1">
        <svg
          width="25px"
          height="25px"
          viewBox="0 0 192 192"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
        >
          <path
            stroke="#000000"
            strokeLinecap="round"
            strokeWidth="12"
            d="M96 65v44m22-22H74"
          />
          <path
            stroke="#000000"
            strokeLinejoin="round"
            strokeWidth="12"
            d="M96 151.5c35.898 0 65-28.99 65-64.75C161 50.99 131.898 22 96 22c-35.898 0-65 28.99-65 64.75 0 42.55 39 74 65 83.25v-18.5Z"
          />
        </svg>
      </div>
    );
  }

  if (icon === 'logo') {
    return (
      <>
        <svg
          viewBox="0 0 369.89473684210526 92.10526315789474"
          preserveAspectRatio="xMidYMid meet"
          className="css-1uxicuq"
          id="accachhh"
        >
          <defs id="SvgjsDefs1170"></defs>
          <g
            id="SvgjsG1171"
            transform="matrix(1.0011441707611084,0,0,1.0011441707611084,-4.505148741418765,-4.004576659038902)"
            fill="#00204a"
          >
            <path
              xmlns="http://www.w3.org/2000/svg"
              fill="#00204a"
              d="M4.5,4v92h2h88h2V4H4.5z M50.5,10.472l32.836,65.671C75.634,85.797,63.781,92,50.5,92  s-25.134-6.203-32.835-15.857L50.5,10.472z M15.035,72.457C10.904,65.957,8.5,58.256,8.5,50c0-22.044,17.075-40.164,38.692-41.857  L15.035,72.457z M53.808,8.143C75.425,9.836,92.5,27.956,92.5,50c0,8.256-2.404,15.957-6.535,22.457L53.808,8.143z M8.5,8h23.274  C21.444,12.624,13.124,20.944,8.5,31.274V8z M12.981,76.564L8.5,85.527V68.726C9.739,71.494,11.243,74.117,12.981,76.564z M9.736,92  l5.977-11.954C20.102,85.12,25.575,89.225,31.774,92H9.736z M69.226,92c6.199-2.774,11.673-6.88,16.062-11.954L91.264,92H69.226z   M92.5,85.527l-4.481-8.963c1.738-2.447,3.242-5.07,4.481-7.839V85.527z M69.226,8H92.5v23.274  C87.876,20.944,79.556,12.624,69.226,8z"
            ></path>
          </g>
          <g
            id="SvgjsG1172"
            transform="matrix(3.4841206073760986,0,0,3.4841206073760986,112,4.227332271091175)"
            fill="#00204a"
          >
            <path d="M0 4.24 l0 15.76 l0.52 0 l0 -15.76 l-0.52 0 z M10.100000000000001 4.58 l-0.000019531 11.56 c0 0.36 0.0066602 0.76 0.02 1.2 s0.02 0.66666 0.02 0.68 c-0.02666 -0.04 -0.12332 -0.25666 -0.28998 -0.65 s-0.33 -0.74334 -0.49 -1.05 l-0.28 -0.54 l-0.72 0 l2.2 4.22 l0.18 0 l0 -15.42 l-0.64 0 z M4.48 7.9399999999999995 l0.27998 0.54 l0.72 0 l-2.22 -4.24 l-0.14 0 l0 15.46 l0.62 0 l0 -11.6 c0 -0.33334 -0.0066602 -0.72334 -0.02 -1.17 s-0.02 -0.67666 -0.02 -0.69 c0.01334 0.04 0.10668 0.26 0.28002 0.66 s0.34 0.74666 0.5 1.04 z M20.32 4.58 l-0.000019531 11.56 c0 0.36 0.0066602 0.76 0.02 1.2 s0.02 0.66666 0.02 0.68 c-0.02666 -0.04 -0.12332 -0.25666 -0.28998 -0.65 s-0.33 -0.74334 -0.49 -1.05 l-0.28 -0.54 l-0.72 0 l2.2 4.22 l0.18 0 l0 -15.42 l-0.64 0 z M14.7 7.9399999999999995 l0.27998 0.54 l0.72 0 l-2.22 -4.24 l-0.14 0 l0 15.46 l0.62 0 l0 -11.6 c0 -0.33334 -0.0066602 -0.72334 -0.02 -1.17 s-0.02 -0.67666 -0.02 -0.69 c0.01334 0.04 0.10668 0.26 0.28002 0.66 s0.34 0.74666 0.5 1.04 z M30.560000000000002 20 c2.2134 0 4.1 -0.78 5.66 -2.34 c0.77334 -0.76 1.3567 -1.58 1.75 -2.46 s0.59 -1.88 0.59 -3 c0 -2.2266 -0.78 -4.1066 -2.34 -5.64 c-1.56 -1.52 -3.4866 -2.2866 -5.78 -2.3 c-1.4133 0.01334 -2.74 0.36668 -3.98 1.06 c-1.2133 0.70666 -2.18 1.6867 -2.9 2.94 l0.58 0.32 c0.65334 -1.16 1.54 -2.06 2.66 -2.7 c1.1067 -0.64 2.32 -0.96666 3.64 -0.98 c2.1334 0.01334 3.9066 0.72668 5.32 2.14 c1.4133 1.3867 2.1266 3.1066 2.14 5.16 c0 1.0267 -0.18 1.9433 -0.54 2.75 s-0.88666 1.5633 -1.58 2.27 c-1.4267 1.4267 -3.1666 2.14 -5.22 2.14 c-1.28 0 -2.44 -0.27334 -3.48 -0.82 l-0.3 0.58 c1.1333 0.58666 2.3934 0.88 3.78 0.88 z M43.24 19.32 c-0.66666 -0.05334 -1.2134 -0.36 -1.64 -0.92 l-0.44 0.42 c0.53334 0.69334 1.2067 1.08 2.02 1.16 z M43.86000000000001 19.98 c0.76 -0.08 1.35 -0.42666 1.77 -1.04 s0.63 -1.44 0.63 -2.48 l0 -12.22 l-0.58 0 l0 12.22 c-0.01334 1.7467 -0.64 2.7 -1.88 2.86 z M55.86000000000001 20 c2.2134 0 4.1 -0.78 5.66 -2.34 c0.77334 -0.76 1.3567 -1.58 1.75 -2.46 s0.59 -1.88 0.59 -3 c0 -2.2266 -0.78 -4.1066 -2.34 -5.64 c-1.56 -1.52 -3.4866 -2.2866 -5.78 -2.3 c-1.4133 0.01334 -2.74 0.36668 -3.98 1.06 c-1.2133 0.70666 -2.18 1.6867 -2.9 2.94 l0.58 0.32 c0.65334 -1.16 1.54 -2.06 2.66 -2.7 c1.1067 -0.64 2.32 -0.96666 3.64 -0.98 c2.1334 0.01334 3.9066 0.72668 5.32 2.14 c1.4133 1.3867 2.1266 3.1066 2.14 5.16 c0 1.0267 -0.18 1.9433 -0.54 2.75 s-0.88666 1.5633 -1.58 2.27 c-1.4267 1.4267 -3.1666 2.14 -5.22 2.14 c-1.28 0 -2.44 -0.27334 -3.48 -0.82 l-0.3 0.58 c1.1333 0.58666 2.3934 0.88 3.78 0.88 z M66.46000000000001 10.94 l0.000039062 9.06 l3.84 0 c1.1467 0 2.0534 -0.40666 2.72 -1.22 s1 -1.8667 1 -3.16 c0 -0.34666 -0.04334 -0.74332 -0.13 -1.19 s-0.23332 -0.88 -0.43998 -1.3 s-0.48666 -0.8 -0.84 -1.14 s-0.81 -0.57666 -1.37 -0.71 c0.56 -0.34666 0.96666 -0.81 1.22 -1.39 s0.38 -1.19 0.38 -1.83 c0 -1.2267 -0.34 -2.17 -1.02 -2.83 s-1.6067 -0.99 -2.78 -0.99 l-2.58 0 l0 6.22 l0.7 0 l-0.02 -5.4 l1.96 0 c1.1067 0 1.8867 0.29 2.34 0.87 s0.68 1.29 0.68 2.13 c0 0.90666 -0.20666 1.61 -0.62 2.11 s-0.98 0.75 -1.7 0.75 l-2.62 0 l0.44 0.88 l2.68 0 c0.54666 0 1.0133 0.12334 1.4 0.37 s0.70332 0.55666 0.94998 0.93 s0.43 0.79334 0.55 1.26 s0.18 0.92 0.18 1.36 c0 0.42666 -0.06 0.84666 -0.18 1.26 s-0.3 0.78334 -0.54 1.11 s-0.55334 0.59 -0.94 0.79 s-0.84666 0.3 -1.38 0.3 l-3.2 0 l0.02 -8.24 l-0.7 0 z"></path>
          </g>
        </svg>
      </>
    );
  }
  return (
    <>
      <svg
        width="25px"
        height="25px"
        viewBox="0 0 192 192"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
      >
        <path
          stroke="#000000"
          strokeLinecap="round"
          strokeWidth="12"
          d="M96 65v44m22-22H74"
        />
        <path
          stroke="#000000"
          strokeLinejoin="round"
          strokeWidth="12"
          d="M96 151.5c35.898 0 65-28.99 65-64.75C161 50.99 131.898 22 96 22c-35.898 0-65 28.99-65 64.75 0 42.55 39 74 65 83.25v-18.5Z"
        />
      </svg>
    </>
  );
}

export default Icon;
